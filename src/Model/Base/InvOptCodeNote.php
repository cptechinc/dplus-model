...
/**
 * Base class that represents a row from the 'notes_opt_code_in' table.
 *
 *
 *
...
abstract class InvOptCodeNote implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Map\\InvOptCodeNoteTableMap';
    ...
    /**
     * The value for the qnoptsys field.
     *
    ...
    /**
     * Get the [qnoptsys] column value.
     *
     * @return string
     */
    public function getQnoptsys()
    {
        return $this->qnoptsys;
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        $sql = sprintf(
            'INSERT INTO notes_opt_code_in (%s) VALUES (%s)',
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'QnOptSys':
                        $stmt->bindValue($identifier, $this->qnoptsys, PDO::PARAM_STR);
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setQntype($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setQntypedesc($arr[$keys[2]]);
        }
    ...
    {
        $pks = [];
        $pks[0] = $this->getQnoptsys();
        $pks[1] = $this->getQntype();
        $pks[2] = $this->getQnseq();
        $pks[3] = $this->getQnkey2();
        $pks[4] = $this->getQnform();
    ...
    {
        $this->setQnoptsys($keys[0]);
        $this->setQntype($keys[1]);
        $this->setQnseq($keys[2]);
        $this->setQnkey2($keys[3]);
        $this->setQnform($keys[4]);
    }
    ...
    {
        return (null === $this->getQnoptsys()) && (null === $this->getQntype()) && (null === $this->getQnseq()) && (null === $this->getQnkey2()) && (null === $this->getQnform());
    }
    ...
    {
        $copyObj->setQnoptsys($this->getQnoptsys());
