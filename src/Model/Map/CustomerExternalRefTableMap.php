...
use \CustomerExternalRef;
use \CustomerExternalRefQuery;
...
/**
 * This class defines the structure of the 'cust_ship_link' table.
 *
 *
 *
...
class CustomerExternalRefTableMap extends TableMap
{
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('CustomerExternalRef');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\CustomerExternalRef');
        $this->setPackage('');
    ...
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \CustomerExternalRef) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \CustomerExternalRef object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if (null !== ($obj = CustomerExternalRefTableMap::getInstanceFromPool($key))) {
            ...
            $col = $offset + CustomerExternalRefTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = CustomerExternalRefTableMap::OM_CLASS;
            /** @var CustomerExternalRef $obj */
            $obj = new $cls();
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = CustomerExternalRefTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = CustomerExternalRefTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(CustomerExternalRefTableMap::COL_ARSTSHIPID);
            $criteria->addSelectColumn(CustomerExternalRefTableMap::COL_CSLKLINKCUSTID);
            $criteria->addSelectColumn(CustomerExternalRefTableMap::COL_CSLKLINKSHIPID);
            $criteria->addSelectColumn(CustomerExternalRefTableMap::COL_DATEUPDTD);
            $criteria->addSelectColumn(CustomerExternalRefTableMap::COL_TIMEUPDTD);
            $criteria->addSelectColumn(CustomerExternalRefTableMap::COL_DUMMY);
        } else {
            $criteria->addSelectColumn($alias . '.ArcuCustId');
            $criteria->addSelectColumn($alias . '.ArstShipId');
            $criteria->addSelectColumn($alias . '.CslkLinkCustId');
            $criteria->addSelectColumn($alias . '.CslkLinkShipId');
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \CustomerExternalRef) { // it's a model object
            ...
            foreach ($values as $value) {
                ...
                $criterion->addAnd($criteria->getNewCriterion(CustomerExternalRefTableMap::COL_ARSTSHIPID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = CustomerExternalRefQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            CustomerExternalRefTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                CustomerExternalRefTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from CustomerExternalRef object
        }
