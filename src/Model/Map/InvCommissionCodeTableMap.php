...
use \InvCommissionCode;
use \InvCommissionCodeQuery;
...
/**
 * This class defines the structure of the 'inv_comm_code' table.
 *
 *
 *
...
class InvCommissionCodeTableMap extends TableMap
{
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('InvCommissionCode');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\InvCommissionCode');
        $this->setPackage('');
  ...
  array (
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        return null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Intbcommgrup', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Intbcommgrup', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Intbcommgrup', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Intbcommgrup', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Intbcommgrup', TableMap::TYPE_PHPNAME, $indexType)];
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        return (string) $row[
            $indexType == TableMap::TYPE_NUM
                ? 0 + $offset
                : self::translateFieldName('Intbcommgrup', TableMap::TYPE_PHPNAME, $indexType)
        ];
    }
    ...
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if (null !== ($obj = InvCommissionCodeTableMap::getInstanceFromPool($key))) {
            ...
            $col = $offset + InvCommissionCodeTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = InvCommissionCodeTableMap::OM_CLASS;
            /** @var InvCommissionCode $obj */
            $obj = new $cls();
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = InvCommissionCodeTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = InvCommissionCodeTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \InvCommissionCode) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(InvCommissionCodeTableMap::DATABASE_NAME);
            $criteria->add(InvCommissionCodeTableMap::COL_INTBCOMMGRUP, (array) $values, Criteria::IN);
        }

        $query = InvCommissionCodeQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            InvCommissionCodeTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                InvCommissionCodeTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from InvCommissionCode object
        }
