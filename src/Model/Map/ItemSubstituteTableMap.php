...
/**
 * This class defines the structure of the 'inv_inv_sub' table.
 *
 *
 *
...
class ItemSubstituteTableMap extends TableMap
{
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('ItemSubstitute');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\ItemSubstitute');
        $this->setPackage('');
        $this->setUseIdGenerator(false);
        // columns
        $this->addForeignPrimaryKey('InitItemNbr', 'Inititemnbr', 'VARCHAR' , 'inv_item_mast', 'InitItemNbr', true, 30, '');
        $this->addForeignPrimaryKey('InsiSubItemNbr', 'Insisubitemnbr', 'VARCHAR' , 'inv_item_mast', 'InitItemNbr', true, 30, '');
        $this->addColumn('InsiSameLike', 'Insisamelike', 'VARCHAR', false, 1, null);
  ...
  array (
    ...
    1 => ':InitItemNbr',
        ...
        $this->addRelation('ItemMasterItemRelatedByInsisubitemnbr', '\\ItemMasterItem', RelationMap::MANY_TO_ONE, array (
  0 =>
  array (
    0 => ':InsiSubItemNbr',
    1 => ':InitItemNbr',
    ...
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \ItemSubstitute) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \ItemSubstitute object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        return serialize([(null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Inititemnbr', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Inititemnbr', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Inititemnbr', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Inititemnbr', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Inititemnbr', TableMap::TYPE_PHPNAME, $indexType)]), (null === $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Insisubitemnbr', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Insisubitemnbr', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Insisubitemnbr', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Insisubitemnbr', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Insisubitemnbr', TableMap::TYPE_PHPNAME, $indexType)])]);
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        $pks[] = (string) $row[
            $indexType == TableMap::TYPE_NUM
                ? 1 + $offset
                : self::translateFieldName('Insisubitemnbr', TableMap::TYPE_PHPNAME, $indexType)
        ];
    ...
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if (null !== ($obj = ItemSubstituteTableMap::getInstanceFromPool($key))) {
            ...
            $col = $offset + ItemSubstituteTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = ItemSubstituteTableMap::OM_CLASS;
            /** @var ItemSubstitute $obj */
            $obj = new $cls();
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = ItemSubstituteTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = ItemSubstituteTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(ItemSubstituteTableMap::COL_INSISUBITEMNBR);
            $criteria->addSelectColumn(ItemSubstituteTableMap::COL_INSISAMELIKE);
            $criteria->addSelectColumn(ItemSubstituteTableMap::COL_DATEUPDTD);
            $criteria->addSelectColumn(ItemSubstituteTableMap::COL_TIMEUPDTD);
            $criteria->addSelectColumn(ItemSubstituteTableMap::COL_DUMMY);
        } else {
            $criteria->addSelectColumn($alias . '.InitItemNbr');
            $criteria->addSelectColumn($alias . '.InsiSubItemNbr');
            $criteria->addSelectColumn($alias . '.InsiSameLike');
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \ItemSubstitute) { // it's a model object
            ...
            foreach ($values as $value) {
                ...
                $criterion->addAnd($criteria->getNewCriterion(ItemSubstituteTableMap::COL_INSISUBITEMNBR, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = ItemSubstituteQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            ItemSubstituteTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                ItemSubstituteTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from ItemSubstitute object
        }
